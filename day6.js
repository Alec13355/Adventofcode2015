...
string = `toggle 461,550 through 564,900
...
// string = 'turn on 222,12 through 856,241';
...
///     x x x x
//      x x x x
// turn on 0,0 through 2,3

lightPos =  Array.from({ length: 1000 }, () => Array(1000).fill(0));
litLights = 0;
str = string.split('\n');
str.forEach(element => {
    ...
    if (command === "turn"){
        command = command + splitString[1];
        start = splitString[2].split(',');
        end = splitString[4].split(',');
        //console.log(start[1]);
    }
    else {
        start = splitString[1].split(',');
        end = splitString[3].split(',');
        //console.log(start[0]);

    }
    for (let i = parseInt(start[1]); i <= parseInt(end[1]); i++) {
        for (let j = parseInt(start[0]); j <= parseInt(end[0]); j++) {
            if(command === 'toggle') {
                lightPos[i][j] = !lightPos[i][j];
